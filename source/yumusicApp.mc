...
class YuMusicApp extends Application.AudioContentProviderApp {
    ...
    private function safe_get_property_boolean(key as String) as Boolean? {
        try {
            return Properties.getValue(key) as Boolean?;
        } catch (ex) {
            return null;
        }
    }
    ...
    function getProviderIconInfo() as Media.ProviderIconInfo? {
        return new Media.ProviderIconInfo(Rez.Drawables.ProviderIcon, 0xC7771F);
    }
